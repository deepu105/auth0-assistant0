...
async function getManagementClient(event, api) {
    ...
    if (!token) {
        ...
        try {
            const domain =  event.secrets?.AUTH0_DOMAIN || event.request.hostname;
            const tokenset = await client.clientCredentialsGrant(config, {
                audience: new URL('/api/v2/', \`https://\${domain}\`).toString(),
            });
...
export const accountLinkingActionTemplate = {
  name: 'account-linking',
  code,
  runtime: 'node22',
  deploy: true, // this will attempt to deploy action after creation
  supported_triggers: [
    {
      id: 'post-login' as const,
      version: 'v3',
...
export const generateAccountLinkingAction = ({
  domain,
  clientId,
  clientSecret,
}: {
